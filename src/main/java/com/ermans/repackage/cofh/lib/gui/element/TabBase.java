...
import com.ermans.repackage.cofh.lib.gui.TabTracker;
...
//        GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0F);
    ...
    public void addTooltip(List<String> list) {
        ...
        for (int i = 0; i < this.elements.size(); i++) {
            ...
            if (!c.isVisible() || !c.intersectsWith(gui.getMouseX() - this.posX(), gui.getMouseY() - this.posY)) {
                continue;
            }
            if (!(c instanceof ElementButtonRedstone)) {
                if (!c.isEnabled()) {
                    continue;
                }
            }
