...
public final class StringHelper {
    ...
    public static List<String> getLinesFromString(int spacePerLine, String text) {
        List<String> lines = new ArrayList<String>();
        int pointer = 0;

        while (pointer + spacePerLine < text.length()) {
            int offset = 0;
            while (text.charAt(pointer + spacePerLine - 1 - offset) != ' ' && offset != spacePerLine) {
                offset++;
            }
            if (offset == spacePerLine) {
                lines.add(text.substring(pointer, pointer + spacePerLine).trim());
                pointer = pointer + spacePerLine;
            } else {
                lines.add(text.substring(pointer, pointer + spacePerLine - offset).trim());
                pointer = pointer + spacePerLine - offset;
            }
        }
        lines.add(text.substring(pointer, text.length()).trim());
    ...
    public static String getFancyNameFromCamel(String name) {
        String fancyName = "";
        if (name.isEmpty()) {
            return name;
        }
        fancyName = upperChar(name.charAt(0)) + fancyName;

        for (int i = 1; i < name.length() - 1; i++) {
            char c = name.charAt(i);
            char cN = name.charAt(i + 1);

            if (isLower(c) && isUpper(cN)) {
                fancyName += c + " ";
            } else {
                fancyName += c;
            }
        ...
        if (name.length() > 1) {
            fancyName += name.charAt(name.length() - 1);
        }
        return fancyName;
    }
