package com.ermans.repackage.cofh.lib.gui;

import com.ermans.repackage.cofh.lib.audio.SoundBase;
...
public abstract class GuiBase extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int x, int y) {
		...
		if (drawTitle & name != null) {
			fontRendererObj.drawString(StringHelper.localize(name), getCenteredOffset(StringHelper.localize(name)), 4, 0x404040);
		}
		if (drawInventory) {
	...
	public void drawItemStack(ItemStack stack, int x, int y, boolean drawOverlay, String overlayTxt) {
		...
		if (drawOverlay) {
			...
			itemRender.renderItemOverlayIntoGUI(font, this.mc.getTextureManager(), stack, x, y - 8, overlayTxt);
		}
